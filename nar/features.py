...
def add_past_form(df: pd.DataFrame, windows=(3, 5, 10)) -> pd.DataFrame:
    ...
    # 3戦1勝を勝率33%として扱うと過大評価になる。ここも縮小する
    p0 = float((1.0 / df["n_starters"].clip(lower=1)).mean())
    df["career_win_rate"] = (
        (df["career_wins"] + PRIOR_K["career"] * p0)
        / (df["career_starts"] + PRIOR_K["career"]))
...
#   IDが無くても名前で名寄せできる。
# =====================================================================
# 経験ベイズの事前重み。n が小さいときに率が0/1に振れるのを抑える。
#   ★これが無いと「1戦1勝の調教師 = 勝率100%」が最強特徴になる★
#   2026-08-15 の学習で trn_win_rate 系が係数1位2位を占め、
#   しかも符号が逆(+0.43 / -0.48)で打ち消し合っていた。過学習の典型。
PRIOR_K = {"jky": 100.0, "trn": 60.0, "career": 8.0}


def _shrink(cnt: pd.Series, n: pd.Series, k: float, p0: float) -> pd.Series:
    """(成功数 + k*p0) / (試行数 + k)。n=0 なら p0 に落ちる。"""
    return (cnt + k * p0) / (n + k)
...
def add_person_form(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # 基準勝率は頭数から出す。結果を一切使わないのでリークしない
    p0 = float((1.0 / d["n_starters"].clip(lower=1)).mean())
    p3 = min(3.0 * p0, 0.9)
    ...
    for col, pre in [("jockey_name", "jky"), ("trainer_name", "trn")]:
        ...
        d[f"{pre}_starts"] = n
        d[f"{pre}_win_rate"] = _shrink(cw, n, PRIOR_K[pre], p0)
        d[f"{pre}_top3_rate"] = _shrink(ct, n, PRIOR_K[pre], p3)
...
# =====================================================================
# ★model.ConditionalLogit はレース内 z化する★
#   つまり素の特徴はすでにレース内相対量になっている。
#   そこに同じ変数の順位を足すと、ほぼ同一方向の列が2本並び、
#   L2正則化下で係数が打ち消し合う(実測: trn_win_rate +0.43 / _rank -0.48)。
#   → 既定は素のみ。順位版は --sweep で効果を測ってから採否を決める。
BASE_COLS = [
    # 枠・斤量
